    ...
    T: 'static + Send + Sync,
{
    fn spawn(
        ...
        let window = EguiWindow::open_parented(
            ...
            move |egui_ctx, queue, state| {
                ...
                if let Some(new_size) = egui_state.new_size.load() {
                    // Ask the plugin host to resize to self.size()
                    if context.request_resize() {
                        // Resize content of egui window
                        queue.resize(new_size.0, new_size.1);
                        ...
                        egui_state.new_size.store(None);
                    }
                }
    ...
    fn size(&self) -> (u32, u32) {
        let new_size = self.egui_state.new_size.load();
